...
nix-store --export $({ nix-store -qR devshells/musl-export; nix-store -qR devshells/default-export; } | sort -u) | gzip > "${OUT_DIR}/devshell.tar.gz"
...
ssh jope9155@klevas.mif.vu.lt "rm ~/www/remu/*"
